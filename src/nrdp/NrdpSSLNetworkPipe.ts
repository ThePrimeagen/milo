...
class NrdpSSLNetworkPipe extends NetworkPipe implements INetworkPipe {
    ...
    constructor(platform: NrdpPlatform, options: ICreateSSLNetworkPipeOptions, callback: (error?: Error) => void) {
        ...
        this.sslInstance = platform.ssl.createSSL((preverifyOk: number, x509StoreContext: N.Struct) => {
            if (preverifyOk !== 1) {
                ...
                const xChain = platform.ssl.X509_STORE_CTX_get0_chain(x509StoreContext);
                if (xChain) {
                    const chain = [];
                    const num = platform.ssl.OPENSSL_sk_num(xChain);
                    for (let i = 0; i < num; ++i) {
                        const x509 = platform.ssl.OPENSSL_sk_value(xChain, i);
                        if (x509) {
    ...
    private _flushOutputBio() {
        ...
        if (pending > 0) {
            ...
            // this.platform.trace("writing", read, platform.ssl.BIO_ctrl_pending(this.outputBio));
    ...
    private _connect() {
        ...
        if (ret <= 0) {
            ...
            if (platform.ssl.SSL_get_error(this.sslInstance, ret) === platform.ssl.SSL_ERROR_WANT_READ) {
                ...
                this.platform.error("BIG FAILURE", platform.ssl.SSL_get_error(this.sslInstance, ret), N.errno);
